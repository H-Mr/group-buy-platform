...
public class XmlUtil {
    ...
    /**
     * <![CDATA[]]> 是 XML 标准的 “原生语法”，所有符合 XML 规范的解析器都必须支持，与框架无关；
     * <!CDATA[...]]> 是 XML（可扩展标记语言）中的一种语法结构，CDATA 代表 "Character Data"（字符数据）。
     * 它用于在 XML 文档中包含不希望被解析器处理的文本数据。
     * <![CDATA[ 和 ]]> 之间的内容会被解析器视为纯文本，而不会被解释为 XML 标记或实体引用。
     * 这在需要包含特殊字符（如 <、>、& 等）或保留字时特别有用，因为这些字符在 XML 中有特殊含义。
     * 使用 CDATA 可以避免对这些字符进行转义，从而简化了文本的编写和阅读。
     * 例如：
     * <example>
     * <data>
     ...
     * </data>
     * </example>
     * 在上面的例子中，CDATA 部分中的文本不会被解析器解释为 XML 标记，而是作为纯文本处理。
     * 解析微信发来的请求(xml)
     */
